...
import {
    getLoggedCompany, getObjects, obterVagaEmEdicao, selecionarVagaParaEdicao, setObject, setObjects,
...
function cadastrarVaga(): void {
    const btnCadastrar = document.querySelector("#cadastrar-vaga") as HTMLButtonElement;

    btnCadastrar.onclick = () => {
        const empresaAtual = getLoggedCompany();
        const nome = document.querySelector("#job-name") as HTMLInputElement;
        const descricao = document.querySelector("#description") as HTMLInputElement;
        const dataPublicacao = document.querySelector("[name='data-publicacao']") as HTMLInputElement;
        ...
        const vaga: Vaga = new Vaga(nome.value, descricao.value, empresaAtual, new Date(dataPublicacao.value), competencias);
...
function adicionaEventoEditarVagas(): void {
    const editar_btn = document.querySelectorAll<HTMLButtonElement>(".editar-vaga");
...
function adicionaEventoDeletarVagas() {
    const deletar_btn = document.querySelectorAll<HTMLButtonElement>(".deletar-vaga");
...
function editarVaga(): void {
    ...
    let descricaoInput = document.querySelector("#description") as HTMLInputElement;
    let dataPublicacaoInput = document.querySelector("[name='data-publicacao']") as HTMLInputElement;
    let btnCadastrar = document.querySelector("#cadastrar-vaga") as HTMLButtonElement;
    ...
    dataPublicacaoInput.value = vaga.data_publicacao ? new Date(vaga.data_publicacao).toISOString().split("T")[0] : "";
    selecionarCompetencias(vaga.competencias)
